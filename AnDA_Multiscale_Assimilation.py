...
""" AnDA_Multiscale_Assimilation.py: Apply multi-scale, patch-based stochastic and sequential data assimilation techniques using analog forecasting. """
...
class Multiscale_Assimilation:
    ...
    def single_patch_assimilation(self,coordinate):
        ...
        if (len(sea_mask)!=self.PR.patch_r*self.PR.patch_c): # bordering patches, reset to classic AF
        ...
        if (len(sea_mask)==self.PR.patch_r*self.PR.patch_c): # not bordering patches
            if (AF.flag_catalog):
        ...
        if (AF.flag_model):
            gr_vl_test_p = VAR.gr_vl_test[k_patch*len(VAR.dX_GT_test):(k_patch+1)*len(VAR.dX_GT_test),:]
        else:
            gr_vl_test_p = None
    ...
    def multi_patches_assimilation(self, saved_path, level, r_start, r_length, c_start, c_length):
        ...
        AnDA_result_test.corr_OI = AnDA_correlate(AnDA_result_test.itrp_OI,AnDA_result_test.GT)
        ...
        AnDA_result_test.corr_AnDA = AnDA_correlate(AnDA_result_test.itrp_AnDA,AnDA_result_test.GT)    
        ...
        with open(saved_path, 'wb') as handle:
            pickle.dump(AnDA_result_test, handle)
